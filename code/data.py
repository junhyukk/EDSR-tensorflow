...
class DataGenerator():
    ...
    def load_data(self):
        ...
        if self.args.is_test:
            ...
            if self.args.data_dir == "Z:/Dataset/SR_test" or self.args.is_test:
                self.args.data_dir = "Z:/Dataset/SR_test"
            ...
            # The list of file_names for each directory 
            self.file_names_for_dirs = [sorted([f for f in os.listdir(dir) if not f=='Thumbs.db'])[:800] for dir in dirs]
    ...
    def get_batch(self, batch_size, idx_scale, in_patch_size=48):
        ...
        for _ in range(batch_size):
            # select image
            idx_img = random.randrange(800) 
            in_img = self.dataset[0][idx_img]
